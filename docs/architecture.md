...
**Routing**: Each domain has its own router module in `routers/` under the `/api/` prefix (`ingredients`, `snacks`, `snack_units`, `recipes`, `trips`, `daily_plan`, `settings`, `food_intake`) — eight routers. `daily_plan` shares the `/api/trips` prefix with `trips`.

**Session management**: Each router defines its own identical `get_db()` generator (no shared utility). Tests override all eight independently via `app.dependency_overrides` (`trips`, `snacks`, `snack_units`, `recipes`, `ingredients`, `daily_plan`, `settings`, `food_intake`).
...
**Services**: A service layer owns workflow and read projections; routers and MCP tools are thin adapters over it. `services/trip_planning.py` (`TripPlanningService`) is the transaction-scoped owner of trip validation, inventory mutation, daily-plan invalidation/regeneration, cloning, and deletion. `services/trip_queries.py` and `services/daily_plan_queries.py` produce the trip overview, summary, packing, shopping, and daily-plan read projections (the packing projection reports each recipe ingredient as a per-serving `amount_oz` plus a combined `total_oz`, so assembly packs one baggie per serving, and on a structured trip adds a `units` assembly checklist grouped by unit type or catalog item — count, per-unit target and derived weight, packed state, actual weight; the shopping projection sums across servings into a single buy-total, expanding each bag selection into its composition ounces so bulk ingredients merge onto one line whatever the source); `services/catalog_queries.py` produces the shared recipe and snack catalog projections (`recipe_list_view`, `snack_view`, `snack_list_view`) that both `/api/recipes` + `/api/snacks` and the MCP `list_food_options` tool return, so `mcp_server.py` imports nothing from `routers.*`; it also owns every snack unit (bag) composition derivation (`snack_unit_composition`, `snack_unit_type_view`, `snack_unit_type_list_view`, `snack_unit_weight_warning`), so no consumer recomputes bag weight, calories, or macros. `services/snack_units.py` sits on top of it for trips: the unit quota (`unit_quota`, per-day round-half-up of `snacks_per_day` x day fraction), the selection projections (`trip_snack_unit_view`, `trip_snack_unit_list_view`), and the totals the summary rolls in (`trip_unit_totals`) — packaged units read catalog serving values, bags read the library's derived values. Supporting calculators remain small and pure: `recipe_calc.py` computes recipe totals, `autofill.py` distributes meals/snacks across trip days and, on structured trips, deals unit selections into the morning/afternoon snack slots against the per-day quota it is handed (`distribute_snack_units`), `calculator.py` implements Skurka method calorie/weight targets.
...
**Dependencies**: Pinned and hash-locked. `requirements.txt` (runtime) and `requirements-dev.txt` (adds the test toolchain) are compiled from `requirements.in` / `requirements-dev.in` and list every transitive dependency with `==` versions and `--hash` entries, so installs are reproducible and integrity-checked rather than resolving unpinned latest. Regenerate with `scripts/update-requirements.sh` (add `--upgrade` / `--upgrade-package NAME` for deliberate bumps). Both the CI gate and the deploy bootstrap install with `pip install --require-hashes`, which fails closed if a package or hash is missing; there is no live-index freshness check, so an upstream publish never blocks a deploy — lock freshness is the developer's responsibility when editing a `.in` file.
...
**MCP**: `mcp_server.py` exposes a compact domain-level FastMCP tool surface over exact-path Streamable HTTP. Tools operate through the service layer (`TripPlanningService`, `catalog_queries`, `snack_units`) against the same transaction boundary as the REST API — no imports from REST router modules; the one exception is `create_snack_unit_type`, which writes its `SnackUnitType`/`SnackUnitIngredient` rows directly because bag creation lives in the router rather than a service, and reads the result back through `catalog_queries`. DNS-rebinding protection is enabled with an env-driven allowed host/origin policy: defaults cover `localhost:8000`, `127.0.0.1:8000`, `beebaby:8000`, and the host/origin of `HIKING_FOOD_OAUTH_ISSUER`, and are replaced wholesale when `HIKING_FOOD_MCP_ALLOWED_HOSTS` / `HIKING_FOOD_MCP_ALLOWED_ORIGINS` are set. OAuth discovery, registration, authorization, and token endpoints live in `mcp_oauth/`; the outer `/hiking-food` ASGI mount becomes the public issuer prefix.
...
**Frontend**: Vitest, Testing Library, jest-dom, and jsdom exercise the rendered application through `BrowserRouter` with mocked HTTP responses at the owned API boundary (`src/test/apiMock.js`). The suite is 9 files / 68 tests and covers routing (direct links, redirects, trip switching, deletion, the remembered last-selected trip, in-place refresh keeping the planner mounted, global pages, not-found states, stale-response races), the `useMutation` hook, per-control pending/error state, per-serving recipe assembly in packing, and accessible control names across meal/snack selection, the calculator, packing, trip create/clone/delete, and the daily plan.
